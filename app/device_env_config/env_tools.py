...
def _method_to_attr(method_suffix):
    """Convert getter/setter suffix to attribute name.

    Handles camelCase with leading acronyms:
      'OSName'      → 'osName'
      'OSVersion'   → 'osVersion'
      'Platform'    → 'platform'
    ...
    upper_count = 0
    for ch in method_suffix:
        if ch.isupper():
            upper_count += 1
        else:
            break
    # 2+ uppercase followed by lowercase → acronym: lowercase all but last uppercase
    if upper_count >= 2 and upper_count < len(method_suffix):
        return method_suffix[: upper_count - 1].lower() + method_suffix[upper_count - 1 :]
    # All uppercase or single uppercase → just lowercase first char
    return method_suffix[0].lower() + method_suffix[1:]


class DeviceEnvBase:
    """Base class for device environment configs (EnvAndroid, EnvIos, etc.).

    Eliminates 20+ boilerplate getter/setter methods via ``__getattr__``.
    Subclasses only need to define:

    1. ``__init__`` with ``self.xxx = value`` for each property
    2. Explicit overrides for methods with custom logic:
       - ``getToken(number)``   — token generation
       - ``getUserAgent()``     — UA string assembly
       - ``getDeviceName2()``   — display-name lookup (iOS) / alias (Android)
       - ``getBuildVersion()``  — dict lookup (iOS only)
       - ``getDeviceModelType()`` — dict lookup (iOS only)

    getXxx() / setXxx() are auto-derived from attribute names:
      getOSName()    → self.osName
      setVersion(v)  → self.version = v
    """
    ...
    def __getattr__(self, name):
        ...
        if name.startswith("get"):
            attr = _method_to_attr(name[3:])
            # Only create getter if the attribute actually exists
